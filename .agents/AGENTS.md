...
| 2026-09-23 | **Botón Atrás en Plantilla de Equipo y Perfil de Jugador**: Añadido botón interactivo `← Atrás` en la vista de detalle de equipo (`/equipos/[teamId]`) y en la vista de perfil de jugador (`/jugadores/[playerId]`). Utiliza `router.back()` si hay historial, con fallback seguro a `/equipos` (o a la página del equipo en el caso del jugador). |
